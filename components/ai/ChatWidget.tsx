...
interface ChatWidgetProps {
  // "floating" (default) is the persistent bottom-right launcher used on every
  // page. "inline" renders just the conversation panel with no fixed
  // positioning and no open/close toggle — for embedding directly in page
  // content (e.g. the homepage hero), so the real live AI is visible on
  // first load instead of hidden behind a small corner button.
...
export function ChatWidget({ variant = "floating" }: ChatWidgetProps) {
  ...
  const conversation = (
    <>
      <div
        ...
        className={
          variant === "inline"
            ? "p-4 flex flex-col gap-3 overflow-y-auto h-[240px]"
            : "p-4 flex flex-col gap-3 overflow-y-auto grow min-h-[220px]"
        }
      >
        {messages.map((m, i) => (
          <div
  ...
  if (variant === "inline") {
    // No header here — the caller (homepage hero) already frames this in its
    // own browser-chrome card with a "zelvoraq-agent · live" label, so this
    // just needs to be the real, working conversation.
    return (
      <div className="bg-elevated border border-borderc rounded-lg2 overflow-hidden flex flex-col">
        {conversation}
      </div>
    );
  }
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
        {!open && (
          <span className="hidden sm:block bg-surface border border-borderc rounded-full px-3.5 py-2 text-[12.5px] text-secondary shadow-soft font-medium whitespace-nowrap">
            Chat with our AI
          </span>
        )}
        <button
          ...
          className="w-14 h-14 rounded-full bg-accent shadow-soft flex items-center justify-center hover:bg-accent-hover transition-colors shrink-0"
        >
          {open ? (
